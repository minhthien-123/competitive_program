...
int solve(int i, int cnt, int mask)
{
    ...
    if (memo[i][cnt][mask] != -1)
    {
        return memo[i][cnt][mask];
    }

    int cur = mask & ((1 << h[i]) - 1);

    ll sum = 0;

    sum = (sum + solve(i + 1, cnt, cur)) % mod;
    ...
    for (int r = 0; r < h[i]; r++)
    {
        ...
        {
            int nmask = cur | (1 << r);
            sum = (sum + solve(i + 1, cnt + 1, nmask)) % mod;
        }
    }

    return memo[i][cnt][mask] = sum;
}
