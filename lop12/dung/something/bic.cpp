...
__Thien_dep_trai__
{
    ...
    for (int u = 1; u <= n; u++)
    {
        ...
        for (int v : adj[u])
        {
            if (flag[v] && tj.scc_id[u] == tj.scc_id[v])
            {
                check = true;
                break;
            }
    ...
    for (int id = 1; id <= tj.scc_cnt; id++)
    {
        for (int u : tj.scc_nodes[id])
        {
            if (!flag[u] || u == 2)
            {
                continue;
    ...
    if (over[1])
    {
        std::cout << std::setfill('0') << std::setw(9) << dp[1] << "\n";
    }
    else
    {
        std::cout << dp[1] << "\n";
