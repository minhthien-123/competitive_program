...
int n, q;
int timer = 0;
int tin[maxn + 7], tout[maxn + 7], depth[maxn + 7];
int up[maxn + 7][lg], mn[maxn + 7][lg];
int bit[maxn + 7];
bool check[maxn + 7];
...
int get_min(int u, int v)
{
    ...
    for (int i = lg - 1; i >= 0; i--)
    {
        if (up[u][i] != up[v][i])
        {
            res = std::min({res, mn[u][i], mn[v][i]});
            u = up[u][i];
            v = up[v][i];
        }
    }
    res = std::min({res, mn[u][0], mn[v][0]});
    return res;
}
...
void solve()
{
    ...
    while (q--)
    {
        ...
        if (type == 1)
        {
            ...
            if (u == v)
            {
                std::cout << (check[u] ? -1 : 0) << "\n";
                continue;
            }
            int lca = get_lca(u, v);
            int get = query(tin[u]) + query(tin[v]) - 2 * query(tin[lca]) + check[lca];
            if (get > 0)
            {
                std::cout << -1 << "\n";
            }
