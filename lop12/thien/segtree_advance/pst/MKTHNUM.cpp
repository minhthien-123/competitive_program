...
long long query(int L, int R, int l, int r, int k)
{
    ...
    int mid = (l + r) / 2;
    int cntL = st[st[R].lc].sum - st[st[L].lc].sum;
    if (cntL >= k)
    {
        return query(st[L].lc, st[R].lc, l, mid, k);
    }
    else
    {
        return query(st[L].rc, st[R].rc, mid + 1, r, k - cntL);
    }
}

struct Compress
{
    // Rời rạc hóa (Coordinate Compression) - Rất cần khi tọa độ lên tới 1e9 nhưng số lượng điểm nhỏ
    // Cách dùng: Compress cmp; cmp.add(val)... cmp.init(); int index = cmp.get(val);
    ...
    int get(long long x)
    {
        // Trả về index 1-based (từ 1 đến số lượng phần tử phân biệt)
...
void solve()
{
    ...
    roots[0] = build(1, cmp.size());

    for (int i = 1; i <= n; i++)
    {
        roots[i] = update(roots[i - 1], 1, cmp.size(), a[i], 1);
    }
    ...
    for (int i = 1; i <= m; i++)
    {
        int l, r, k;
        std::cin >> l >> r >> k;
        int pos = query(roots[l - 1], roots[r], 1, cmp.size(), k);
        std::cout << cmp.vals[pos - 1] << "\n";
    }
}
