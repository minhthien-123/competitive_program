...
struct TrieXor
{
    ...
    int insert(int old, int x)
    {
        ...
        for (int i = 29; i >= 0; i--)
        {
            ...
            child[curr][0] = child[old][0];
            child[curr][1] = child[old][1];

            child[curr][bit] = ++node_cnt;
            curr = child[curr][bit];
            old = child[old][bit];
            cnt[curr]++;
        }
    ...
    int query_min(int root, int x)
    {
        ...
        for (int i = 29; i >= 0; i--)
        {
            ...
            {
                ans |= (1LL << i);
                curr = child[curr][1 - bit];
            }
        }
        return ans;
    }

    int query_max(int root, int x)
    {
        int curr = root;
        int ans = 0;
        for (int i = 29; i >= 0; i--)
        {
            int bit = (x >> i) & 1;
            int target = 1 - bit;
            if (child[curr][target])
            {
                ans |= (1LL << i);
                curr = child[curr][target];
            }
            else
            {
                curr = child[curr][bit];
