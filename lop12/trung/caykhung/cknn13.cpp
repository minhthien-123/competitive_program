...
int n;
ii px[maxn + 7], py[maxn + 7], pz[maxn + 7];
...
void solve()
{
    ...
    std::sort(pz + 1, pz + n + 1);

    std::vector<edge> edges;
    for (int i = 1; i < n; i++)
    {
        edges.push_back({px[i].se, px[i + 1].se, px[i + 1].fi - px[i].fi});
        edges.push_back({py[i].se, py[i + 1].se, py[i + 1].fi - py[i].fi});
        edges.push_back({pz[i].se, pz[i + 1].se, pz[i + 1].fi - pz[i].fi});
    }
    std::sort(all(edges));
